...
* 🐍 **What are Functions?** - General functions explained
* 🐍 **Functions Without Return Values** - Quick action-oriented example
* 🐍 **Functions That Return Values** - Quick value-producing example
...
A function in Python is defined using the `def` keyword, followed by the function name, parentheses for parameters, and a colon. The body of the function is indented.

:::

```python
def function_name(parameters):
    # The function body uses parameters to perform actions
    # that create local variables, calculations, etc.
    # A variable may be returned as necessary.

    return value  # Return the value if you want to send something back to the caller
...
* The function performs an action: returns a value or prints a message.
* The function is called by its name: `function_name()`. 
* A value is returned if `return` is used; otherwise, Python returns `None`.


## The `say_hello()` Function Example
...
def say_hello(to_name):
    ...
    print(f"Hello, {to_name}! Welcome to Python functions.")
...
* It does not use `return`, so Python gives back `None` (if we try to check the return value).
...
<center>
![](graphics/cheesy_smile.png){width="50%"}
</center>

::: {style="color: #2E86C1; font-size: 0.9em; text-align: center; margin-top: 0.5em;"} 

<!-- ::: {.callout} -->
Like, what are functions that return values?
<!-- ::: -->
...
myWidth = 5
myLength = 3
area = area_of_rectangle(myLength, myWidth)
print(area)
```
<center></center>
<center></center>
...
<center>
![](graphics/rectangleArea.png){width="30%"}
</center>
...
With `key=`, `max()` can choose the “largest” item using custom logic.
:::
...
<!-- 
## More Helpful Functions continued! 🎯
...
**❌ DO NOT**: Make lambdas too complex or use them for multi-step logic
```python
